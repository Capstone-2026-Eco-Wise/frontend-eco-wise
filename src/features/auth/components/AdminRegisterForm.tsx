...
import { MdEmail, MdPerson, MdVpnKey } from "react-icons/md";
import { useAdminRegisterState } from "../hooks/useAdminRegisterState";
...
export default function AdminRegisterForm() {
  const {
    ...
    error,
    setError,
  } = useAdminRegisterState();

  const { handleRegister } = useAdminRegister({
    fullName,
    username,
    email,
    password,
  ...
  const onSubmitHandler = async (event: FormEvent) => {
    ...
    try {
      ...
      setError(error.message || "Registrasi admin gagal! Silakan coba lagi.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <form onSubmit={onSubmitHandler} className="flex flex-col gap-5 w-full">
      ...
      <div className="mb-1">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1.5">
          Daftar sebagai administrator EcoWise.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="relative">
          <Input
            ...
            className="h-11 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 pr-10 pl-3 rounded-xl text-sm focus-visible:ring-emerald-500 focus-visible:border-emerald-500"
          />
          <MdVpnKey className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 size-4 pointer-events-none" />
        </div>
      </div>
      ...
      >
        {loading ? "Membuat akun admin..." : "Daftar Admin"}
      </button>
    </form>
  );
}
